...
def main():
    ...
    output_folder = reference_path + pas_dataset + "/distribution/"
    ...
    # Use 161 as the length for -400 to 400 and 86 for -400 to 25
    hit_list = np.array([0] * 65)
    ...
    for row in reader:
        ...
        if len(pas_hits) == 0:
            ...
            for hit_dist in pas_hits:
                hit_index = (304 + hit_dist) // 5
                hit_list[hit_index] += 1

    # Stores our data as a numpy array to be concatenated with other numpy arrays by the head script.
    # Our data is stored in the distribution/ folder of our PAS dataset.
